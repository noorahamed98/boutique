...
- Admin viewport-fit pass: the studio modal now uses a true desktop viewport shell when opened on laptop/desktop widths, locks the background page scroll while the admin is open, keeps the login/dashboard content denser so it fits the visible screen better, shows the top stats in a single row on wide screens, and keeps unavoidable overflow inside the internal admin panels instead of the whole page.
