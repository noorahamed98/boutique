...
- Collection image cleanup: collection cards and the design modal now use an `AdaptiveDesignImage` component that classifies uploads as wide/tall/standard from their natural aspect ratio. Wide collage uploads keep all looks visible with a framed contain treatment, portrait shots keep a fuller cover-style presentation, and the collection grid now uses `auto-fit` so filtering down to two items no longer leaves an empty third column.
- Admin viewport-fit pass: the studio modal now locks the background page scroll while open, uses a viewport-bounded desktop shell instead of taking over the full browser area, keeps the login/dashboard denser for laptop-height screens, and hides lower-priority sidebar sections on shorter desktop heights so the main console matches the intended screen-fit composition more closely.
- Admin laptop-height compact mode: for short desktop screens around the user's laptop height, the authenticated admin console now uses an even tighter density pass with a shorter header, smaller stat cards, more compact overview/meta blocks, tighter search/manage rows, smaller form controls, and a lighter gap system so the first dashboard content fits the visible area with less clipping.
- Admin compact follow-up: the short-laptop mode now keeps the `Storefront categories` sidebar card visible again, hides only the lower-priority `Recent updates` block on shorter desktop heights, prevents the left sidebar track from stretching into dead empty space, and trims the top header/stat spacing further so the desktop console composition matches the user's reference more closely.
- Mobile responsive pass: the latest storefront and admin changes now have dedicated phone/tablet overrides at the end of `client/src/styles.css`. On mobile, the main site uses tighter shell padding, simpler one-column content grids, better stacked catalog/modals, and a denser hero/menu treatment. The admin now opens as a full-screen mobile sheet with stacked sections, two-column stat cards on larger phones, single-column category/manage cards on narrow phones, and touch-friendly action buttons/forms.
