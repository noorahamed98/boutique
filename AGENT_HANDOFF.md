...
- Admin viewport-fit pass: the studio modal is now vertically constrained to the screen on desktop, centers within the viewport instead of starting at the top, uses a more compact login/dashboard spacing scale, shows the top stats in a single row on wide screens, and keeps any extra overflow inside the internal admin panels rather than forcing whole-page up/down scrolling.
