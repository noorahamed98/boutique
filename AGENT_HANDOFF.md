...
- Admin viewport-fit pass: the studio modal now uses a near-fullscreen laptop-fit shell on desktop, keeps the login/dashboard content denser so it fills the screen without looking cut off, shows the top stats in a single row on wide screens, and keeps any unavoidable overflow inside the internal admin panels instead of the whole page.
